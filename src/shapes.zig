...
pub const Shape = struct {
    ...
    insideFnPtr: *const fn (self: *const Shape, point: Vec3f.Type) bool,
    deinitFnPtr: *const fn (ptr: *const Shape) void,
    originFnPtr: *const fn (ptr: *const Shape) Vec3f.Type,
    ...
    pub fn inside(self: *const Shape, point: Vec3f.Type) bool {
    ...
    pub fn origin(self: *const Shape) Vec3f.Type {
...
pub const Sphere = struct {
    ...
    pub fn init(allocator: Allocator, center: Vec3f.Type, radius: fsize, mat: Material) !*Sphere {
    ...
    pub fn inside(ptr: *const Shape, point: Vec3f.Type) bool {
        const self: *const Sphere = @ptrCast(@alignCast(ptr.ptr));

        const positionCenter = self.center - point;
        const distance = Vec3f.dot(positionCenter, positionCenter);
    ...
    pub fn intersection(ptr: *const Shape, ray: Ray, rayT: Interval) ?HitRecord {
        ...
        const a = Vec3f.lengthSq(ray.direction);
        const h = Vec3f.dot(ray.direction, oc);
        const c = Vec3f.lengthSq(oc) - (self.radius * self.radius);
        ...
        const outwardNormal = (pos - self.center) / Vec3f.S(self.radius);
        return HitRecord.init(pos, time, ray, outwardNormal, self.mat);
    }

    pub fn origin(ptr: *const Shape) Vec3f.Type {
        const self: *const Sphere = @ptrCast(@alignCast(ptr.ptr));
        return self.center;
    }
...
test "Create Sphere" {
    const originTest = Vec3f.Type{ 34, 0.003, -1 };
    const radius = 5.6;
    ...
    try testing.expectEqual(Vec3f.Type{ 34, 0.003, -1 }, sphereTest.center);
}

test "Inside Origin" {
    const originTest = Vec3f.Type{ 0, 0, 0 };
    const radius = 1;
    const sphereTest = try Sphere.init(talloc, originTest, radius);
    defer sphereTest.destroy(talloc);

    const ITV = struct { expected: bool, vector: Vec3f.Type };

    const insideTest = [_]ITV{
        .{ .expected = false, .vector = Vec3f.Type{ 0, 0, 1.00001 } },
        .{ .expected = true, .vector = Vec3f.Type{ 0, 0, 1.0 } },
        .{ .expected = true, .vector = Vec3f.Type{ 0.577350, -0.577350, 0.577350 } },
        .{ .expected = false, .vector = Vec3f.Type{ -0.577351, -0.577350, 0.577350 } },
    };
...
test "Inside Moved" {
    const originTest = Vec3f.Type{ 3.5, 2.4, 5.7 };
    const radius = 0.59;
    const sphereTest = try Sphere.init(talloc, originTest, radius);
    defer sphereTest.destroy(talloc);
    ...
    const insideTest = [_]ITV{
        .{ .expected = true, .vector = Vec3f.Type{ 3.5, 2.4, 5.7 } },
        .{ .expected = true, .vector = Vec3f.Type{ 3.5, 1.82, 5.7 } },
        .{ .expected = false, .vector = Vec3f.Type{ 0.1, 2.4, 5.7 } },
        .{ .expected = true, .vector = originTest + (Vec3f.unit(Vec3f.Type{ 0.25, 0.8, -2.2 }) * Vec3f.S(radius)) },
        .{ .expected = false, .vector = originTest + (Vec3f.unit(Vec3f.Type{ 0.25, 0.8, -2.2 }) * Vec3f.S(radius * 1.0001)) },
    };
